...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    bool_t ovf = FALSE;
    if(op == A_ADD)
		ovf = (int)argA < 0 && (int) argB < 0 && (int) val > 0; 
    if(op == A_SUB)
		ovf = (int)argA > 0 && (int) argB < 0 && (int) val > 0;
...
stat_t nexti(y64sim_t *sim)
{
    ...
    switch(icode){
    	...
    	case I_PUSHQ:
    		if (!get_byte_val(sim->m, next_pc, &regs)) {
        		err_print("PC = 0x%lx, Invalid registers address", sim->pc);
        		return STAT_ADR;
    		}
    ...
    bool_t setCond;
    switch (icode) {
      case I_HALT: /* 0:0 */ //TODO: what happened to pc here
	    return STAT_HLT;
	    break;
      case I_NOP: /* 1:0 */
    	sim->pc = next_pc;
      ...
      case I_RRMOVQ:  /* 2:x regA:regB */
        if(cond_doit(sim->cc,(cond_t)ifun)){
            regA_val = get_reg_val(sim->r, regA.id);
    	    set_reg_val(sim->r, regB.id, regA_val);
        }
    	sim->pc = next_pc;
    	break;
      ...
      case I_RMMOVQ: /* 4:0 regA:regB imm */
        regB_val = get_reg_val(sim->r, regB.id);
        regA_val = get_reg_val(sim->r, regA.id);
    	if(!set_long_val(sim->m, regB_val + imm, regA_val)){
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, regB_val + imm);
            return STAT_ADR;
        }
    	sim->pc = next_pc;
    	break;
      case I_MRMOVQ: /* 5:0 regB:regA imm */
        regB_val = get_reg_val(sim->r, regB.id);
    	if(!get_long_val(sim->m,  regB_val + imm, &val)){
    		err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, regB_val + imm);
   			return STAT_ADR;
    	}
    	set_reg_val(sim->r, regA.id, val);
    	sim->pc = next_pc;
    	break;
      case I_ALU: /* 6:x regA:regB */
        regB_val = get_reg_val(sim->r, regB.id);
        regA_val = get_reg_val(sim->r, regA.id);
    	val = compute_alu(ifun,regA_val,regB_val);
    	sim->cc = compute_cc(ifun, regA_val, regB_val, val);
    	set_reg_val(sim->r, regB.id, val);
    	sim->pc = next_pc;
    	break;
      case I_JMP: /* 7:x imm */
    	if(cond_doit(sim->cc,(cond_t)ifun))
      ...
      case I_PUSHQ: /* A:0 regA:F */
        regA_val = get_reg_val(sim->r, regA.id);
    	rsp = get_reg_val(sim->r, REG_RSP);
    	setCond = set_long_val(sim->m, rsp - 8, regA_val);
    	set_reg_val(sim->r, REG_RSP, rsp - 8);
    	if(!setCond){
    		err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, rsp -8);
   			return STAT_ADR;
